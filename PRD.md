...
| Academic & Teaching Administration (Curriculum → CP → TP → ATP → Prota → Prosem → Teaching Modules → Daily Journals) | V5 | **In progress** — Steps 0-2e and Phases 5A-5E built, through Prosem; Teaching Modules, Daily Journals and teacher student-scoping not started |
...
- **Academic & Teaching Administration (V5)** — Curriculum, Capaian Pembelajaran (CP), Tujuan Pembelajaran (TP), Alur Tujuan Pembelajaran (ATP), Program Tahunan (Prota), Program Semester (Prosem), Teaching Modules (Modul Ajar), Daily Teacher Journal (Jurnal Harian Guru), and teaching-administration dashboards. **Built through Prosem (Phases 5A–5E); Teaching Modules, Daily Journals and the dashboards await explicit go-ahead.** *Correcting the original proposal:* it anchored everything to the existing `class_subject` teaching-assignment record. That proved wrong twice. The standards layers (Curriculum → CP → TP → ATP) are anchored to a **curriculum scope + subject**, so one Phase C pathway serves Year 5 and Year 6 without duplication; and the planning layers (Prota → Prosem) are anchored to the **roster** — a class or a teaching group — so a plan survives a mid-year teacher handover. Only Assessment hangs off the teaching assignment itself. Each layer owns exactly one kind of fact: ATP the sequence, Prota the period and JP budget, Prosem the within-period schedule, Module the "how", Journal the "actual".
