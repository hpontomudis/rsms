...
| Last updated | 2026-08-16 (V9.0 Phase V9A built — AI infrastructure + Communication Draft Assistant; AI is optional assistance, never the source of truth) |
...
| AI-Assisted Management (provider-neutral infrastructure, Communication Draft Assistant) | V9 | **Phase V9A complete** — governing rule: AI MAY ASSIST THE USER; AI MUST NOT BECOME THE SOURCE OF TRUTH. AI never writes to a canonical model; suggested text is applied only by copying into unsaved form state, then saved through the same unmodified save path a human edit uses. Daily Journal AI, Teaching Module AI, Management Insights, Performance Evaluation AI, Report Card AI, RAG/vector search and autonomous agents are all out of scope for this phase |
...
| V9.0 | AI-Assisted Management | **Phase V9A complete.** AI is optional assistance, never the source of truth. A provider-neutral abstraction (`AiProvider` interface, fake-first testing, zero new Composer dependency — the real adapter calls Anthropic over Laravel's `Http` facade) backs the only user-facing feature, a Communication draft rewrite assistant: draft-only, requires the same authorization a manual edit already requires, sends only title/body/mode/language (no student/guardian/staff PII), never mutates the canonical Communication, and is rate-limited (5/minute, 50/day) and logged to a metadata-only `ai_generations` table (no prompt, response or cost persisted). Daily Journal AI, Teaching Module AI, Management Insights, Performance Evaluation AI, Report Card AI, Curriculum Q&A, RAG/vector search and autonomous agents remain future, unapproved phases |
...
- AI-Assisted Management (V9): **Phase V9A complete** (provider-neutral infrastructure, Communication Draft Assistant). Daily Journal AI, Teaching Module AI, Management Insights (read-only, query-grounded — never a free-form number generator), Performance Evaluation AI, Report Card AI, Curriculum Q&A, RAG/vector search, AI search, text-to-SQL and autonomous agents all remain future, additive phases requiring their own explicit approval — none is implied by V9A's infrastructure existing.
