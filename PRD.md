...
| Last updated | 2026-08-16 (V9.0 Phase V9A built — AI infrastructure, Communication Draft Assistant, Daily Journal Reflection Assistant, Teaching Module AI Planning Assistant; AI is optional assistance, never the source of truth) |
...
| AI-Assisted Management (provider-neutral infrastructure, Communication Draft Assistant, Daily Journal Reflection Assistant, Teaching Module AI Planning Assistant) | V9 | **Phase V9A complete** (V9A-1, V9A-2, V9A-3, V9A-4) — governing rule: AI MAY ASSIST THE USER; AI MUST NOT BECOME THE SOURCE OF TRUTH. AI never writes to a canonical model; suggested text is applied only by copying into unsaved form state, then saved through the same unmodified save path a human edit uses. AI designs lessons around a canonical TP but never selects, replaces, removes, or invents one. Management Insights, Performance Evaluation AI, Report Card AI, RAG/vector search and autonomous agents are all out of scope for this phase |
...
| V9.0 | AI-Assisted Management | **Phase V9A complete (V9A-1, V9A-2, V9A-3, V9A-4).** AI is optional assistance, never the source of truth. A provider-neutral abstraction (`AiProvider` interface, fake-first testing, zero new Composer dependency — the real adapter calls Anthropic over Laravel's `Http` facade) backs three user-facing features, all draft-only, all requiring the same authorization a manual edit already requires, all rate-limited (5/minute, 50/day, shared) and logged to one metadata-only `ai_generations` table (no prompt, response or cost persisted): a Communication draft rewrite assistant (title/body/mode/language only), a Daily Journal reflection assistant (two structured fields — `reflection`/`follow_up`, never `actual_activity`), and a Teaching Module planning assistant (five structured fields — `planned_activity`/`teaching_strategy`/`resources`/`differentiation`/`planned_assessment` — designed around an already-linked, canonical Learning Objective it can never select, replace, remove, or invent). Both the Journal and Module assistants use structured JSON output, with a proven, reusable pattern distinguishing provider-transport success from assistant-level result usability. Both also have a finalized/frozen-state firewall deliberately stricter than the underlying policy: a manager may still manually correct a finalized Journal or edit a Ready Module's `teacher_notes`, but AI assistance is refused in both cases regardless. Management Insights, Performance Evaluation AI, Report Card AI, Curriculum Q&A, RAG/vector search and autonomous agents remain future, unapproved phases |
...
- AI-Assisted Management (V9): **Phase V9A complete** (provider-neutral infrastructure, Communication Draft Assistant, Daily Journal Reflection Assistant, Teaching Module AI Planning Assistant). Management Insights (read-only, query-grounded — never a free-form number generator), Performance Evaluation AI, Report Card AI, Curriculum Q&A, RAG/vector search, AI search, text-to-SQL and autonomous agents all remain future, additive phases requiring their own explicit approval — none is implied by V9A's infrastructure existing.
