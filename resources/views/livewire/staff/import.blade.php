<div class="mx-auto max-w-4xl space-y-4">
    ...
    <h1 class="font-serif text-xl font-bold text-brand-navy">Import Staff</h1>

    @if (empty($imported))
        <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            <h2 class="mb-2 text-sm font-semibold text-slate-700">1. Download the template</h2>
            <p class="mb-3 text-sm text-slate-500">Fill it in outside RSMS, then come back and upload it. Creates new Staff only -- rows matching an existing email or NIK are rejected, not overwritten.</p>
            ...
            <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                ...
                <div class="max-h-96 overflow-y-auto">
                    <table class="w-full text-left text-sm">
                        <thead class="text-slate-500">
                            <tr><th class="py-1 pr-3">Row</th><th class="py-1 pr-3">Name</th><th class="py-1 pr-3">Email</th><th class="py-1">Status</th></tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach ($preview as $row)
                                <tr>
                                    ...
                                    <td class="py-1 pr-3">{{ $row->data['email'] ?? '—' }}</td>
                                    <td class="py-1">
                                        @if ($row->isValid())
                ...
                @else
                    <button
                        ...
                        wire:confirm="Import {{ count($preview) }} Staff records?"
    ...
    @else
        <div class="rounded-xl bg-green-50 p-5 ring-1 ring-green-200">
            ...
            <p class="text-sm text-green-800">{{ count($imported) }} Staff records created, {{ count($credentials) }} login accounts provisioned.</p>
        </div>

        @if (count($credentials) > 0)
            <div class="rounded-xl bg-amber-50 p-5 ring-1 ring-amber-200">
                <h2 class="mb-1 text-sm font-semibold text-amber-900">Temporary Credentials</h2>
                <p class="mb-3 text-sm text-amber-800">This file contains sensitive login credentials. Download it now and hand it to each Staff member securely -- it will not be available again.</p>
                <button type="button" wire:click="downloadCredentials" class="rounded-md bg-brand-navy px-5 py-2.5 text-sm font-medium text-white hover:bg-brand-navy-light">
                    Download Credentials (.xlsx)
                </button>
            </div>
        @endif

        <a href="{{ route('staff.index') }}" class="inline-block text-sm text-slate-500 hover:text-slate-700">&larr; Back to Staff</a>
    @endif
</div>
