<div class="mx-auto max-w-xl space-y-4">
    <a href="{{ route('classes.show', $schoolClass) }}" class="text-sm text-slate-500 hover:text-slate-700">&larr; {{ $schoolClass->name }}</a>
    ...
    <form wire:submit="save" class="space-y-4 rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <div>
            ...
            <input type="text" wire:model="name" class="w-full rounded-md border border-slate-300 px-3 py-2.5 text-base">
        ...
        <div class="flex justify-end gap-3 pt-2">
            <a href="{{ route('classes.show', $schoolClass) }}" class="rounded-md border border-slate-300 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50">Cancel</a>
