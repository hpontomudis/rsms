...
Final matrix: `admin_staff` → `teacher` only (both provision and reset). `super_admin` → `teacher`/`admin_staff`/`finance_staff`/`management`/`principal` (both provision and reset) — never `super_admin` through either path; the P1 bootstrap command remains the sanctioned way to (re-)establish a `super_admin` login. Target-role resolution fails closed on ambiguity (zero or multiple roles), never guessed.

**`principal` added to the `super_admin` provision/reset lists** (post-P2.1, deployment-era fix): a school has exactly one principal, and creating or recovering that login was previously unreachable through any path at all — not even by `super_admin`, since P2.1 had deliberately excluded it alongside `super_admin` itself. Unlike `super_admin`, a `principal` login has no separate bootstrap command, so `super_admin` resetting one through the ordinary Staff UI is the only way a locked-out principal recovers access. `admin_staff` still cannot provision or reset a `principal`, and `super_admin` still cannot provision or reset another `super_admin` — both unchanged.
